...
public class UserController{
    ...
    public UserController(CreateUserUseCase createUserUseCase, TaxNumberAvailableUseCase taxNumberAvailableUseCase, EmailAvailableUseCase emailAvailableUseCase, TransactionPinValidateUseCase transactionPinValidateUseCase, UserMapper userMapper) {
    ...
    public BaseResponse<String> createUser(@Valid @RequestBody CreateUserRequest request) throws Exception, TransactionalException {
        ...
        var encryptedTransactionPin = transactionPinValidateUseCase.validate(request.pin());
        taxNumberAvailableUseCase.taxNumberAvaliable(request.taxNumber());
        emailAvailableUseCase.emailAvaliable(request.email());
        createUserUseCase.create(userMapper.toUser(request), encryptedTransactionPin);
